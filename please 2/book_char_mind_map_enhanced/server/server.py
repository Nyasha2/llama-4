...
You are an expert literary translator with deep knowledge of over 200 languages. Your task is to translate an entire book while preserving its literary quality, character voices, cultural nuances, and narrative style.

**Objective:** Translate the provided book text into the target language while maintaining:
- Character personalities and speaking styles
- Plot structure and pacing
- Cultural context (adapting when necessary)
- Literary quality and narrative flow
- Dialogue authenticity

**Instructions:**

1. **Preserve Character Voices:** Each character should have a distinct voice in the target language
2. **Cultural Adaptation:** Adapt cultural references when direct translation wouldn't make sense
3. **Maintain Tone:** Keep the same emotional weight and atmosphere
4. **Dialogue Authenticity:** Make conversations sound natural in the target language
5. **Literary Quality:** Maintain the original's literary merit and style

**Important Notes:**
- If cultural references need adaptation, choose equivalent concepts from the target culture
- Preserve names unless they have specific meanings that should be translated
- Keep the same chapter/section structure
- Maintain the same reading level and complexity

**Output Format:** Complete translated text that reads naturally in the target language while staying true to the original story.

Translate with the skill of a professional literary translator, ensuring the story remains engaging and authentic in the new language.
"""

SETTING_TRANSFORMATION_SYSTEM_PROMPT = """
You are an expert story adaptation AI. Your task is to transform a story's setting (time period, location, or cultural context) while preserving the core plot, character relationships, and narrative structure.

**Objective:** Adapt the provided story to the new setting while maintaining:
- Core plot structure and major events
- Character personalities and relationships
- Emotional beats and story themes
- Narrative pacing and tension

**Transformation Guidelines:**

1. **Time Period Changes:**
   - Update technology, social norms, language patterns
   - Adapt historical/cultural references
   - Modify character occupations and lifestyles
   - Update dialogue to match the era's speaking patterns

2. **Location Changes:**
   - Adapt geographical and cultural references
   - Change architecture, climate, local customs
   - Update food, clothing, transportation
   - Modify social structures and institutions

3. **Cultural Context Changes:**
   - Adapt social hierarchies and relationships
   - Update cultural practices and beliefs
   - Modify character backgrounds and motivations
   - Change institutions and power structures

**Important Notes:**
- Keep character personalities and relationships intact
- Preserve the story's emotional core and themes
- Maintain plot structure - only adapt HOW events happen, not WHAT happens
- Ensure all changes feel natural and consistent
- Update dialogue to match the new setting's linguistic patterns

**Output Format:** Complete adapted story that feels authentic to the new setting while preserving the original's narrative essence.

Create a seamless adaptation that makes readers feel the story naturally belongs in the new setting.
"""
...
def generate_contextual_choices():
    ...
    try:
        ...
        # Build enhanced prompt with language and setting context
        language_instruction = ""
        if target_language != 'English':
            language_instruction = f"\n**CRITICAL: Generate all choice text and descriptions in {target_language}. The character actions and dialogue should be in {target_language}.**"
        
        setting_instruction = ""
        if setting_context:
            setting_instruction = f"\n**Setting Context: {setting_context}** - Ensure all choices reflect this setting's technology, social norms, and cultural context."
        
        prompt = f"""
        Character: {character}
        
        Current Scene: {scene_context}{language_instruction}{setting_instruction}
...
def continue_story_enhanced():
    ...
    try:
        ...
        language_instruction = ""
        if target_language != 'English':
            language_instruction = f"\n**CRITICAL: Write the entire response in {target_language}. All dialogue, narration, and descriptions must be in {target_language}.**"
        
        setting_instruction = ""
        if setting_context:
            setting_instruction = f"\n**Setting Context: {setting_context}** - Maintain this setting's tone, technology level, social norms, and dialogue patterns throughout the continuation."
        ...
        Original Story Style Reference: {original_style}{language_instruction}{setting_instruction}
        ...
        Maintain the original author's writing style and character voices while preserving any language or setting adaptations.
        """
...
@app.route("/translate_book", methods=["POST"])
def translate_book():
    """
    Translates the entire book into the specified language
    """
    try:
        data = request.json
        if not data or 'book_content' not in data or 'target_language' not in data:
            return jsonify({"error": "book_content and target_language are required"}), 400

        book_content = data['book_content']
        target_language = data['target_language']
        
        # Split book into chunks for translation (to handle token limits)
        chunk_size = 15000  # Adjust based on token limits
        chunks = [book_content[i:i+chunk_size] for i in range(0, len(book_content), chunk_size)]
        
        translated_chunks = []
        
        for i, chunk in enumerate(chunks):
            print(f"Translating chunk {i+1}/{len(chunks)} into {target_language}")
            ...
            Translate this text into {target_language} while preserving the literary quality, character voices, and narrative style. Maintain the same emotional weight and make dialogue sound natural in the target language.
            """
            ...
            translated_chunk = call_llama_api(messages, max_tokens=20000, temperature=0.3)
            
            if not translated_chunk:
                return jsonify({"error": f"Failed to translate chunk {i+1}"}), 500
        ...
        return jsonify({
            "translated_content": translated_book,
...
@app.route("/transform_setting", methods=["POST"])
def transform_setting():
    """
    Transforms the book's setting while preserving the core story
    """
    try:
        data = request.json
        if not data or 'book_content' not in data or 'setting_type' not in data:
            return jsonify({"error": "book_content and setting_type are required"}), 400
        ...
        # Create setting description based on type
        if setting_type == 'original':
            return jsonify({
                "transformed_content": book_content,
                "setting_description": "Original setting maintained",
                "status": "success"
            }), 200
        elif setting_type == 'time':
            setting_description = f"Time period changed to: {time_period}"
        elif setting_type == 'place':
            setting_description = f"Location changed to: {location}"
        elif setting_type == 'time_and_place':
            setting_description = f"Setting changed to: {time_period} in {location}"
        elif setting_type == 'custom':
            setting_description = f"Custom setting: {custom_setting}"
        else:
            return jsonify({"error": "Invalid setting_type"}), 400
        
        # Split book into chunks for transformation
        chunk_size = 12000  # Smaller chunks for complex transformations
        chunks = [book_content[i:i+chunk_size] for i in range(0, len(book_content), chunk_size)]
        
        transformed_chunks = []
        
        for i, chunk in enumerate(chunks):
            print(f"Transforming chunk {i+1}/{len(chunks)} to new setting")
            
            prompt = f"""
            Setting Transformation: {setting_description}
            Time Period: {time_period if time_period else 'Not specified'}
            Location: {location if location else 'Not specified'}
            Custom Setting: {custom_setting if custom_setting else 'Not specified'}
            
            Original text to transform:
            {chunk}
            
            Transform this text to the new setting while preserving:
            - Core plot structure and events
            - Character personalities and relationships
            - Emotional beats and themes
            - Narrative pacing
            
            Adapt technology, dialogue, cultural references, and social norms to fit the new setting naturally.
            """
            
            messages = [
                {"role": "system", "content": SETTING_TRANSFORMATION_SYSTEM_PROMPT},
                {"role": "user", "content": prompt}
            ]
            
            transformed_chunk = call_llama_api(messages, max_tokens=20000, temperature=0.4)
            
            if not transformed_chunk:
                return jsonify({"error": f"Failed to transform chunk {i+1}"}), 500
        ...
        return jsonify({
            "transformed_content": transformed_book,
        ...
        print(f"Error transforming setting: {str(e)}")
        return jsonify({"error": str(e)}), 500
...
def upload_book():
    """
    Step 1: Just upload and read the book content without any analysis
    """
    try:
        if 'file' not in request.files:
        ...
        # Extract a simple title from the first few lines or filename
