  ...
  **Character:** Sarah Kim
  ...
  const verificationGraphData = (graphData) => {
    try {
      ...
      if (!graphData || !graphData.nodes || !graphData.links) {
        console.warn("Invalid graph data provided:", graphData);
        return {
          nodes: [],
          links: []
        };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if queryResult is valid and not an error string
      if (typeof queryResult === 'string' || !queryResult || !queryResult.title) {
        console.error("Invalid response from server:", queryResult);
        alert("Failed to process the book. Please check if the server is running and try again.");
        return;
      }
