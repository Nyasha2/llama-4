...
import EnhancedModeConfig from '../../components/EnhancedModeConfig';

export default function StoryMode() {
  const [step, setStep] = useState(1); // 1: upload, 2: configure, 3: character select, 4: story play
  ...
  const handleEnhancedConfig = (config) => {
    setEnhancedConfig(config);
    setIsEnhancedMode(true);
    // Analyze characters with transformed content
    analyzeBookContent(config.transformedContent);
  };

  const handleSkipEnhancedMode = () => {
    // Use original content without transformation
    setIsEnhancedMode(false);
    analyzeBookContent(originalBookContent);
  };

  const analyzeBookContent = async (contentToAnalyze) => {
    setIsLoading(true);
    try {
      // Create a virtual file object with the content to analyze
      const blob = new Blob([contentToAnalyze], { type: 'text/plain' });
      const virtualFile = new File([blob], 'book_content.txt', { type: 'text/plain' });
      
      const formData = new FormData();
      formData.append('file', virtualFile);

      const response = await axios.post("http://localhost:5002/inference", formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      if (response.data.graph_data && response.data.graph_data.nodes) {
        setCharacters(response.data.graph_data.nodes);
        setBookContent(contentToAnalyze);
        
        // Analyze character appearances
        const appearanceResponse = await axios.post("http://localhost:5002/analyze_character_appearances", {
          book_content: contentToAnalyze,
          characters: response.data.graph_data.nodes
        });
        
        setAppearanceAnalysis(appearanceResponse.data.appearance_analysis);
        setStep(3); // Move to character selection
      }
    } catch (error) {
      console.error("Error analyzing book content:", error);
      alert("Failed to analyze book content. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    setIsLoading(true);
    try {
      // Just read the file content and move to configuration
      const text = await fileObject.text();
      setOriginalBookContent(text);
      setBookContent(text);
      setStep(2); // Move to configuration step
    } catch (error) {
      console.error("Error reading book file:", error);
      alert("Failed to read the book file. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const selectCharacter = async (character) => {
    ...
    try {
      // Get the story segment for this character
              const segmentResponse = await axios.post("http://localhost:5002/get_story_segment", {
      ...
      if (storySegment.includes("SUMMARY:") || storySegment.includes("summary")) {
                  const summaryResponse = await axios.post("http://localhost:5002/get_chapter_summary", {
          book_content: bookContent,
          character: character.name,
  ...
  const generateContextualChoices = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:5002/generate_contextual_choices", {
        ...
        target_language: enhancedConfig.language,
        setting_context: enhancedConfig.settingContext
      });
  ...
  const makeChoice = async (choice) => {
    try {
      setIsLoading(true);
      const response = await axios.post("http://localhost:5002/continue_story_enhanced", {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-100 py-8 px-4">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center">
            ...
            {isEnhancedMode && (
              <div className="text-sm text-purple-600 font-medium mt-1">
                🚀 Enhanced Mode
                {enhancedConfig.language !== 'English' && (
                  <span className="ml-2">🌍 {enhancedConfig.language}</span>
                )}
                {enhancedConfig.settingContext && (
                  <span className="ml-2">🏛️ {enhancedConfig.settingContext}</span>
                )}
              </div>
            )}
            {!isEnhancedMode && (
        ...
        {step === 1 && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            ...
            <form onSubmit={handleFileUpload} className="space-y-6">
              ...
              >
                {isLoading ? "Reading book file..." : "Continue to Configuration"}
              </button>
            </form>
          </div>
        )}

        {/* Step 2: Enhanced Mode Configuration */}
        {step === 2 && (
          <div className="space-y-6">
            {/* Enhanced Mode Configuration */}
            <EnhancedModeConfig
              onConfigApply={handleEnhancedConfig}
              bookContent={originalBookContent}
              isLoading={isLoading}
              setIsLoading={setIsLoading}
            />
            
            {/* Skip Enhanced Mode Option */}
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h3 className="text-lg font-bold mb-4 text-center text-gray-700">
                Or Continue with Original Settings
              </h3>
              <p className="text-center text-gray-600 mb-4">
                Skip enhanced mode and use the story in its original language and setting.
              </p>
              <button
                onClick={handleSkipEnhancedMode}
                disabled={isLoading}
                className="w-full bg-gray-600 text-white py-3 rounded-lg font-semibold hover:bg-gray-700 disabled:opacity-50"
              >
                {isLoading ? "Analyzing story..." : "Continue with Original Mode"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-6">
            {/* Enhanced Mode Status in Character Selection */}
            {isEnhancedMode && (
              <div className="bg-gradient-to-r from-purple-100 to-blue-100 border-2 border-purple-300 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-semibold text-purple-800">Story Mode Configuration</h3>
                    <div className="flex items-center space-x-4 text-sm text-purple-700">
                      {enhancedConfig.language !== 'English' && (
                        <span>🌍 {enhancedConfig.language}</span>
                      )}
                      {enhancedConfig.settingContext && (
                        <span>🏛️ {enhancedConfig.settingContext}</span>
                      )}
                      {!enhancedConfig.settingContext && enhancedConfig.language === 'English' && (
                        <span>📖 Original with Enhanced Features</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {step === 4 && (
          <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg shadow-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="font-bold text-lg">Enhanced Mode Active</h3>
                      <div className="flex items-center space-x-4 text-sm">
                        {enhancedConfig.language !== 'English' && (
                          <div className="flex items-center">
                            <span className="mr-1">🌍</span>
                            <span>Language: {enhancedConfig.language}</span>
                          </div>
                        )}
                        {enhancedConfig.settingContext && (
                          <div className="flex items-center">
                            <span className="mr-1">🏛️</span>
                            <span>Setting: {enhancedConfig.settingContext}</span>
                          </div>
                        )}
                        {!enhancedConfig.settingContext && enhancedConfig.language === 'English' && (
            ...
            {/* Original Mode Status (for clarity) */}
            {!isEnhancedMode && (
              <div className="bg-gray-100 border-2 border-gray-300 rounded-lg p-3">
                <div className="flex items-center">
                  <div className="text-gray-500 mr-2">📖</div>
                  <div>
                    <span className="font-semibold text-gray-700">Original Mode</span>
                    <span className="text-sm text-gray-600 ml-2">• English • Original Setting</span>
                  </div>
                </div>
              </div>
            )}
